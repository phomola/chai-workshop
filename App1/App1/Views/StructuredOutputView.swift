...
//  App1
//
//  Created by Petr Homola on 02/09/2026.
//
...
struct StructuredOutputView: View {
    @State private var prompt = "What are the coordinates of Asunción, Paraguay?"
    ...
    private func submitPrompt() {
        Task {
            ...
            do {
                let response = try await session.respond(to: prompt, generating: Coordinates.self)
                let encoder = JSONEncoder()
                encoder.outputFormatting = [.prettyPrinted, .sortedKeys]
                let json = try encoder.encode(response.content)
                self.response = String(data: json, encoding: .utf8)
            } catch {
